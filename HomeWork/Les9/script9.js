...
block.style.marginTop = '5px'// Щоб не зливалося до кучі)
...
for (const item of simpsons) {
    ...
    h2.innerText = `Age - ${item.age} Year`
...
for (const item of coursesArray) {
    ...
    for (const module of item.modules) {
        let moduLi = document.createElement('li');
        moduLi.classList.add('moduLi')
        moduLi.innerText = `${module}`
        modulesUl.append(moduLi)
        modules.append(modulesUl);
        wrap.append(modulesUl)
    }
...
const form = document.forms.audit;
